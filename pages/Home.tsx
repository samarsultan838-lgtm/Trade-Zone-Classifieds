...
const Home: React.FC = () => {
  ...
  return (
    <div className="space-y-10 pb-16 animate-in fade-in duration-500">
      ...
      <section className="relative rounded-[40px] md:rounded-[56px] overflow-hidden bg-emerald-950 px-4 py-16 md:py-24 text-center">
        <div className="absolute inset-0 opacity-20">
          <img 
            ...
            alt="Luxury Architecture Cityscape" 
            fetchPriority="high"
            width="1200"
            height="800"
          />
        ...
        <div className="relative z-10 max-w-5xl mx-auto">
          <div className="inline-flex items-center gap-2 px-3 py-1.5 bg-emerald-500/10 border border-emerald-500/20 rounded-full text-[9px] font-black uppercase tracking-[0.3em] text-emerald-400 mb-5">
            <Sparkles className="w-3.5 h-3.5" aria-hidden="true" /> Premium Marketplace
          </div>
          ...
          <form onSubmit={handleSearch} className="bg-white/10 backdrop-blur-3xl border border-white/20 p-1.5 rounded-[24px] md:rounded-[30px] max-w-3xl mx-auto flex flex-col md:flex-row items-center gap-1.5 shadow-2xl" role="search">
            <div className="flex-1 w-full relative group">
              <Search className="absolute left-5 top-1/2 -translate-y-1/2 w-4 h-4 text-emerald-400" aria-hidden="true" />
              <input 
                type="text" 
      ...
      {/* CATEGORIES */}
      <section className="max-w-7xl mx-auto px-4" aria-label="Quick Category Search">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {categories.map((cat, i) => (
            <Link key={i} to={`/search?category=${cat.name}`} className="group bg-white p-5 rounded-[24px] border border-gray-100 shadow-sm flex items-center gap-4 hover:border-emerald-200 transition-all">
              <div className={`${cat.color} w-10 h-10 rounded-xl flex items-center justify-center shrink-0`} aria-hidden="true">{cat.icon}</div>
      ...
      <section className="max-w-7xl mx-auto px-4">
        <div className="flex items-center justify-between mb-6">
          ...
          <Link to="/search" className="text-[10px] font-black uppercase text-emerald-600 tracking-widest hover:underline">View Ledger</Link>
        </div>
